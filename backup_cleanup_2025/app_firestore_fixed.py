#!/usr/bin/env python3
"""
LINE Article Intelligence Hub - Firestore Version (Fixed & Simplified)
"""
...
<body>
    <div class="card">
        <h1>📚 Article Hub</h1>
        <p>Save and manage your articles</p>
...
<body>
    ...
    <script>
        ...
        async function loadArticles(stage) {
            try {
                const res = await fetch(`/api/articles?stage=${stage}`);
                const articles = await res.json();
                const container = document.getElementById('articles');
                if (articles.length === 0) {
                    container.innerHTML = 'No articles';
                    return;
                }
                container.innerHTML = articles.map(a => `
                    <div class="article-item" onclick="window.open('${a.url}', '_blank')">
...
<head>
    ...
    <style>
        ...
        .column-header.inbox { background: #e3f2fd; }
        .column-header.reading { background: #fff3e0; }
        .column-header.reviewing { background: #f3e5f5; }
        .column-header.completed { background: #e8f5e9; }
        .article-card {
            background: #f5f5f5;
            padding: 10px;
            margin-bottom: 10px;
...
<body>
        ...
        async function loadKanban() {
            ...
            for (const stage of stages) {
                try {
                    ...
                    container.innerHTML = articles.map(a => `
                        <div class="article-card" draggable="true" data-id="${a.id}" data-stage="${stage}">
                            <strong>${a.title}</strong><br>
                            <small>${a.url}</small>
                        </div>
                    `).join('');
                ...
                zone.addEventListener('drop', async (e) => {
                    ...
                    if (draggedElement) {
                        ...
                        if (oldStage !== newStage) {
                            try {
                                await fetch(`/api/articles/${articleId}/stage`, {
                                    method: 'POST',
    ...
    def serve_login(self):
        """Serve simplified LINE Login page"""
        state = secrets.token_urlsafe(32)
        nonce = secrets.token_urlsafe(32)
        
        # Store in session for verification (in production, use proper session store)
        redirect_uri = f"https://article-hub-959205905728.asia-northeast1.run.app/callback"
        
        # Encode redirect_uri properly
        ...
        # Show a simple page with login button instead of immediate redirect
        html = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="login-card">
        <h1>📱 LINE Login</h1>
        <p>Sign in with your LINE account</p>
        <a href="{login_url}" class="login-btn">Login with LINE</a>
    </div>
</body>
</html>"""
    ...
    def handle_login_callback(self, parsed_path):
        ...
        redirect_uri = "https://article-hub-959205905728.asia-northeast1.run.app/callback"
